 ...
 */
export function cacheKey(entity, identifier) {
    return `cep:${entity}:${identifier}`;
}
/**
 * Generate list cache key
 */
export function listCacheKey(entity, filters) {
 ...
 */
export async function invalidateCache(entity, identifier) {
    const client = getRedis();
    if (!client)
        return;
    try {
 ...
 */
export async function withCache(entity, key, loader) {
 ...
 */
export async function invalidateRelated(entity) {
    const relations = {
