...
class PayloadMock {
    data = new Map();
    nextId = 1;
    logs = []; // Capture logs for PII testing
    constructor() {
    ...
    checkUpdatePermissions(user, collection, data) {
        if (!user || collection !== 'students') {
            return;
        }
        const userRole = user.role;
 ...
 * @returns JWT authentication token
 */
export async function loginAsRole(app, role) {
    const credentials = {
 ...
 */
export async function createTestCycle(payload) {
 ...
 */
export async function createTestCampus(payload) {
    return await payload.create({
        collection: 'campuses',
        data: {
            slug: 'test-campus',
 ...
 */
export async function createTestCourse(payload, cycleId) {
    return await payload.create({
        collection: 'courses',
        data: {
            slug: 'test-course',
