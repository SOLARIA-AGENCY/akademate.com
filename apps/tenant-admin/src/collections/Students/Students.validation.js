...
export const phoneSchema = z.string().regex(spanishPhoneRegex, 'Phone must be in Spanish format: +34 XXX XXX XXX');
// Broad IPv4/IPv6 matcher compatible with Zod v3/v4 (avoids .ip() runtime method differences)
const ipAddressRegex = /^(?:(?:\d{1,3}\.){3}\d{1,3}|(?:[A-Fa-f0-9]{1,4}:){2,7}[A-Fa-f0-9]{1,4})$/;
/**
 ...
 * - formatSpanishPhone('+34612345678') → '+34 612 345 678'
 */
export const formatSpanishPhone = (phone) => {
 ...
 */
export const validateDNIChecksum = (dni) => {
 ...
 * Student status enum
 */
export const VALID_STUDENT_STATUSES = ['active', 'inactive', 'suspended', 'graduated'];
 ...
 * Gender enum
 */
export const VALID_GENDERS = ['male', 'female', 'non-binary', 'prefer-not-to-say'];
 ...
 */
export const safeValidateStudentData = (data) => {
 ...
 */
export const checkEmailUniqueness = async (email, payload, excludeId) => {
 ...
 * This is handled by Payload's unique constraint
 */
export const checkDNIUniqueness = async (dni, payload, excludeId) => {
    const existing = await payload.find({
        collection: 'students',
        where: {
            dni: { equals: dni },
