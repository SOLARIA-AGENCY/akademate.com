import { spanishPhoneRegex, dniRegex, validateDNIChecksum, emailSchema, dateOfBirthSchema, } from '../Students.validation';
/**
    ...
    try {
        ...
        if (data?.phone) {
            if (!spanishPhoneRegex.test(data.phone)) {
                validationErrors.push('Phone must be in Spanish format: +34 XXX XXX XXX (e.g., +34 612 345 678)');
            }
        }
        ...
        if (data?.emergency_contact_phone) {
            if (!spanishPhoneRegex.test(data.emergency_contact_phone)) {
                validationErrors.push('Emergency contact phone must be in Spanish format: +34 XXX XXX XXX');
            }
        }
        ...
        if (data?.emergency_contact_name || data?.emergency_contact_phone) {
            if (!data?.emergency_contact_relationship) {
                validationErrors.push('Emergency contact relationship is required when emergency contact information is provided');
            }
        }
...
//# sourceMappingURL=validateStudentData.js.map