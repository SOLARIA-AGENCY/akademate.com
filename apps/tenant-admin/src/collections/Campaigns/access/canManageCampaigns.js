 ...
 */
export const canManageCampaigns = ({ req: { user } }) => {
    // Not authenticated
    if (!user)
        return false;
    // Admin, Gestor, and Marketing can manage campaigns
    return (user.role === ROLES.ADMIN || user.role === ROLES.GESTOR || user.role === ROLES.MARKETING);
};
//# sourceMappingURL=canManageCampaigns.js.map