 ...
 */
export const validateEnrollmentRelationships = async ({ data, req, operation, }) => {
    if (!data) {
        return data;
    }
    ...
    if (data.course_run) {
        try {
            ...
            if (courseRun.status !== 'enrollment_open') {
                throw new Error(`Course run ${data.course_run} is not accepting enrollments. Current status: ${courseRun.status}`);
            }
        }
        catch (error) {
...
//# sourceMappingURL=validateEnrollmentRelationships.js.map