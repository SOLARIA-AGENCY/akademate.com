 ...
 */
export const enrollmentRefinedSchema = enrollmentCreateSchema.refine((data) => {
 ...
 */
export function isValidEnrollmentStatus(value) {
    return VALID_ENROLLMENT_STATUSES.includes(value);
}
/**
 * Type guard to check if a value is a valid payment status
 */
export function isValidPaymentStatus(value) {
    return VALID_PAYMENT_STATUSES.includes(value);
}
/**
 * Type guard to check if a value is a valid financial aid status
 */
export function isValidFinancialAidStatus(value) {
    return VALID_FINANCIAL_AID_STATUSES.includes(value);
}
//# sourceMappingURL=Enrollments.validation.js.map