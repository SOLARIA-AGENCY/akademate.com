...
export const folderSchema = z
    .string()
    .regex(/^[a-z0-9]+([/-][a-z0-9]+)*$/, 'Folder must be lowercase alphanumeric with hyphens/slashes only (e.g., courses/images)')
    .refine((folder) => !folder.includes('..'), 'Folder cannot contain directory traversal sequences (..)')
    .refine((folder) => !folder.startsWith('/') && !folder.endsWith('/'), 'Folder cannot start or end with slashes')
    .optional();
/**
...
export const filenameSchema = z
    ...
    .trim()
    .refine((filename) => !filename.includes('..'), 'Filename cannot contain directory traversal sequences (..)')
    .refine((filename) => !filename.includes('\x00'), 'Filename cannot contain null bytes')
    .refine((filename) => !/[\x00-\x1F\x7F]/.test(filename), 'Filename cannot contain control characters')
    .refine((filename) => !filename.startsWith('/') && !filename.startsWith('\\'), 'Filename cannot start with path separators');
/**
...
export const mimeTypeSchema = z
    .string()
    .refine((mimeType) => ALLOWED_MIME_TYPES.includes(mimeType), `MIME type must be one of: ${ALLOWED_MIME_TYPES.join(', ')}`)
    .refine((mimeType) => !BLOCKED_MIME_TYPES.includes(mimeType), 'Executable and script file types are not allowed for security reasons');
/**
 ...
 */
export const validateFilenameExtension = (filename, mimeType) => {
