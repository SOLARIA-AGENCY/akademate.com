...
describe('Media Collection', () => {
    ...
    describe('CRUD Operations', () => {
        ...
        it('should delete a media file (Admin only)', async () => {
            ...
            await expect(payload.findByID({ collection: 'media', id: created.id })).rejects.toThrow();
        });
    ...
    describe('Hooks', () => {
        ...
        it('should NOT log sensitive file content in hooks (SP-004)', async () => {
            ...
            const hasFileContent = logCalls.some((call) => JSON.stringify(call).includes('sensitive-content'));
        ...
        it('should log only non-sensitive metadata (filename, size, type)', async () => {
            ...
            const logCalls = consoleLogSpy.mock.calls.flat();
            const hasMetadata = logCalls.some((call) => JSON.stringify(call).includes('metadata-test.png') ||
    ...
    describe('Relationships', () => {
        ...
        it('should handle user deletion gracefully (SET NULL or CASCADE)', async () => {
            ...
            if (mediaAfterUserDeletion) {
                // SET NULL behavior
                expect(mediaAfterUserDeletion.created_by === null ||
                    mediaAfterUserDeletion.created_by === tempUser.id).toBe(true);
            }
        ...
        it('should allow querying media with multiple relationship conditions', async () => {
            ...
            expect(result.docs.length).toBeGreaterThan(0);
            expect(result.docs.every((doc) => doc.created_by === adminUser.id && doc.folder === 'test-folder')).toBe(true);
        });
        it('should support OR conditions in relationship queries', async () => {
            ...
            expect(result.docs.length).toBeGreaterThan(0);
            expect(result.docs.some((doc) => doc.created_by === adminUser.id || doc.created_by === gestorUser.id)).toBe(true);
        });
    });
    // ============================================================================
    // 7. SECURITY (15+ tests)
    ...
    describe('Security', () => {
        ...
        it('SP-004: should NOT log file content (only metadata)', async () => {
            ...
            const logCalls = consoleLogSpy.mock.calls.flat();
            const hasFileContent = logCalls.some((call) => JSON.stringify(call).includes('confidential-file-content-12345'));
            expect(hasFileContent).toBe(false);
            consoleLogSpy.mockRestore();
        });
