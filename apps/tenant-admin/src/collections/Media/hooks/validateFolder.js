...
export const validateFolder = ({ value, data, operation }) => {
    ...
    if (folder.includes('..')) {
        throw new Error('Invalid folder path: Directory traversal sequences (..) are not allowed for security reasons');
    }
    ...
    if (!/^[a-z0-9]+([/-][a-z0-9]+)*$/.test(folder)) {
        throw new Error('Invalid folder path: Must be lowercase alphanumeric with hyphens/slashes only (e.g., courses/images)');
    }
    // ============================================================================
...
//# sourceMappingURL=validateFolder.js.map