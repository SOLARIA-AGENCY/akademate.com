...
import { ALLOWED_MIME_TYPES, BLOCKED_MIME_TYPES, DANGEROUS_EXTENSIONS, MAX_FILE_SIZE, validateFilenameExtension, sanitizeFilename, } from '../Media.validation';
...
export const validateMediaFile = async ({ data, req: _req, operation }) => {
    ...
    if (sanitized.includes('..')) {
        throw new Error('Invalid filename: Directory traversal sequences (..) are not allowed for security reasons');
    }
    ...
    for (const ext of DANGEROUS_EXTENSIONS) {
        ...
        if (pattern.test(lowerFilename)) {
            throw new Error(`Security Error: Filename contains dangerous extension ${ext}. Please remove ${ext} from the filename entirely. This includes patterns like ${ext}-file, file${ext}.png, or file${ext}`);
        }
    }
    // ============================================================================
    ...
    if (BLOCKED_MIME_TYPES.includes(mimeType)) {
        throw new Error(`Security Error: MIME type ${mimeType} is not allowed. Executable and script files are blocked for security reasons.`);
    }
        ...
        try {
            ...
            if (!detectedType) {
                ...
                if (mimeType !== 'image/svg+xml') {
                    throw new Error('Security Error: Unable to determine file type from content. The file may be corrupted or in an unsupported format.');
                }
            }
            else {
                // File has magic bytes - verify they match declared MIME type
                if (detectedType.mime !== mimeType) {
                    throw new Error(`Security Error: File content mismatch. You declared this as ${mimeType}, but the actual file content is ${detectedType.mime}. This is a security risk (MIME type spoofing).`);
                }
            ...
            throw new Error('File validation failed: Unable to verify file content. Please ensure the file is not corrupted.');
        }
    }
    // ============================================================================
    ...
    if (mimeType === 'image/svg+xml' && data.file && data.file.data) {
        try {
            ...
            if (svgContent.includes('<!ENTITY') || svgContent.includes('<!DOCTYPE')) {
                throw new Error('Security Error: SVG files with DTD declarations or external entities are not allowed. This prevents XXE (XML External Entity) attacks.');
            }
            ...
            if (!sanitizedSVG || sanitizedSVG.trim().length === 0) {
                throw new Error('Security Error: SVG sanitization removed all content. The file may contain only malicious code.');
            }
            ...
            throw new Error('SVG validation failed: Unable to sanitize SVG content. Please ensure the file is a valid SVG.');
        }
    }
    // ============================================================================
    ...
    if (filesize > MAX_FILE_SIZE) {
        ...
        throw new Error(`File too large: ${actualSizeMB}MB exceeds maximum allowed size of ${maxSizeMB}MB`);
    }
    // ============================================================================
...
//# sourceMappingURL=validateMediaFile.js.map