 ...
 */
function normalizeSpanishChars(str) {
    const map = {
        á: 'a',
        é: 'e',
        í: 'i',
 ...
 */
async function slugExists(slug, req, currentId) {
    try {
        ...
        if (currentId) {
            return existing.docs.some((doc) => doc.id !== currentId);
        }
 ...
 */
async function generateUniqueSlug(baseSlug, req, currentId) {
    let slug = baseSlug;
    let counter = 1;
 ...
 * Generate slug hook
 */
export const generateSlug = async ({ data, req, operation, value }) => {
