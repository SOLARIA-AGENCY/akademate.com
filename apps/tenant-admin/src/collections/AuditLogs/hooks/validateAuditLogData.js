 ...
 */
export const validateAuditLogData = async ({ data, req, operation, }) => {
    ...
    try {
        ...
        if (data?.collection_name) {
            const isValidCollection = VALID_COLLECTION_NAMES.includes(data.collection_name);
            if (!isValidCollection) {
                throw new Error(`Invalid collection_name: "${data.collection_name}". ` +
        ...
        if (data?.user_id && payload) {
            try {
                ...
                throw new Error(`Invalid user_id: ${error instanceof Error ? error.message : 'User not found'}`);
            }
        }
        // ============================================================================
...
//# sourceMappingURL=validateAuditLogData.js.map