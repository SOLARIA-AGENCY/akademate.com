...
import { uploadAdVideo } from '../../../../../src/lib/meta-marketing'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploaded = await uploadAdVideo(metaContext.meta.adAccountIdNormalized, metaContext.meta.marketingApiToken, absoluteUrl)
    if (!uploaded.success) {
      return NextResponse.json({ success: false, error: { message: uploaded.error || 'No se pudo subir vídeo a Meta.' } }, { status: 502 })
    }

    return NextResponse.json({ success: true, data: { media_id: mediaId, video_id: uploaded.data?.id, source_url: absoluteUrl }, source_health: health })
  } catch (error) {
    return NextResponse.json({ success: false, error: { message: error instanceof Error ? error.message : 'Error subiendo vídeo a Meta.' } }, { status: 500 })
  }
}
