...
export async function getReadinessResponse(): Promise<NextResponse> {
  ...
  if (!sql) {
    return NextResponse.json(
      { status: 'not_ready', check: 'ready', checks: { database: 'not_configured' } },
  ...
  try {
    await sql`SELECT 1`;
    return NextResponse.json(
      { status: 'ready', check: 'ready', checks: { database: 'ok' } },
      { status: 200, headers: getHealthHeaders() },
    );
  } catch {
    return NextResponse.json(
      { status: 'not_ready', check: 'ready', checks: { database: 'unavailable' } },
      { status: 503, headers: getHealthHeaders() },
    );
  }
}
