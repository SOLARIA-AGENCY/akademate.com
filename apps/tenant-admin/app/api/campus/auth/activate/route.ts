...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const authToken = await queryFirst<CampusAuthToken>(
    ...
    if (!authToken || authToken.purpose !== 'setup') return NextResponse.json({ success: false, error: 'Enlace no valido' }, { status: 404 })
    ...
    console.error('[Campus Activate] Error:', error)
    return NextResponse.json({ success: false, error: 'No se pudo activar el acceso' }, { status: 500 })
  }
}
