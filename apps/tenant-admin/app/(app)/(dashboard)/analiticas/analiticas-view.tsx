...
export type AnalyticsSurface = 'marketing' | 'web'

const TABS_BY_SURFACE: Record<AnalyticsSurface, Tab[]> = {
  web: ['overview', 'organic'],
  marketing: ['facebook', 'google'],
}

const SURFACE_TITLE: Record<AnalyticsSurface, string> = {
  web: 'Analíticas web',
  marketing: 'Analíticas de marketing',
}
...
function tabsForSurface(surface: AnalyticsSurface) {
  const allowed = new Set(TABS_BY_SURFACE[surface])
...
export function AnaliticasView({ surface }: { surface: AnalyticsSurface }) {
  const router = useRouter()
  const visibleTabs = useMemo(() => tabsForSurface(surface), [surface])
  const [activeTab, setActiveTab] = useState<Tab>(TABS_BY_SURFACE[surface][0])
  ...
  return (
    <div className="space-y-6">
        ...
        actions={
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <SegmentedToggle
              ...
              value={dateRange}
              onValueChange={(next) => setDateRange(next as DateRange)}
            ...
            <ToggleGroup
              ...
              variant="outline"
              size="sm"
              value={effectiveTrafficGranularity}
              onValueChange={(next) => {
                if (next) setTrafficGranularity(next as TrafficGranularity)
              }}
              aria-label="Granularidad de tráfico"
              className="inline-flex h-auto min-h-10 flex-wrap rounded-md border border-input bg-background p-0.5"
            >
      ...
      {!loading && !error && data && surface === 'web' && activeTab === 'overview' && (
        <div className="space-y-6">
      ...
      {!loading && !error && data && surface === 'web' && activeTab === 'organic' && (
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">
      ...
      {!loading && !error && data && surface === 'marketing' && activeTab === 'facebook' && (
        <div className="space-y-6">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Campañas Facebook</CardTitle>
            </CardHeader>
            <CardContent>
      ...
      {!loading && !error && data && surface === 'marketing' && activeTab === 'google' && (
        <Card>
          <CardHeader>
