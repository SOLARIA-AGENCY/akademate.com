...
function resolveRelationIds(value: unknown): string[] {
  if (!Array.isArray(value)) return []
  return value.map(resolveRelationId).filter(Boolean)
}
...
export default function EditarSedePage({ params }: EditSedePageProps) {
    ...
    async function fetchCampus() {
      try {
        ...
        if (Array.isArray(data.classrooms)) {
          setClassrooms(
            data.classrooms.map((c: Record<string, unknown>) => ({
        ...
        if (data.parking && typeof data.parking === 'object') {
          const p = data.parking as Record<string, unknown>
          setParkingAvailable(p.available === true)
          if (p.spaces) setParkingSpaces(String(p.spaces))
          setParkingFree(p.free === true)
          setParkingNotes((p.notes as string) || '')
        }

        // Schedule
        if (data.schedule && typeof data.schedule === 'object') {
          const s = data.schedule as Record<string, unknown>
          setScheduleWeekdays((s.weekdays as string) || '')
          setScheduleSaturday((s.saturday as string) || '')
          setScheduleSunday((s.sunday as string) || '')
          setScheduleNotes((s.notes as string) || '')
        }

        // Image
        const mediaUrl = resolveMediaUrl(data.image)
        if (mediaUrl) setImagePreview(mediaUrl)
        const imgId = resolveRelationId(data.image)
        if (imgId) setUploadedImageId(Number(imgId))
        ...
        // Asignaciones
        setCoordinator(resolveRelationId(data.coordinator))
        setStaffMembers(resolveRelationIds(data.staff_members))
        setCyclesOffered(resolveRelationIds(data.cycles_offered))
        setCoursesOffered(resolveRelationIds(data.courses_offered))
      } catch (err) {
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground">Cargando datos de la sede...</p>
        </div>
      </div>
    )
  }
