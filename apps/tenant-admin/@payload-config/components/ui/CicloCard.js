...
export function CicloCard({ ciclo, className }) {
    const router = useRouter();
    return (<Card className={`cursor-pointer hover:shadow-xl transition-all duration-300 overflow-hidden border-2 border-gray-200 col-span-2 ${className}`} onClick={() => router.push(`/ciclos/${ciclo.id}`)}>
      <CardContent className="p-0">
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="flex gap-2">
            <Button className={`flex-1 ${ciclo.color} hover:opacity-90 text-white font-bold uppercase`} onClick={(e) => {
            e.stopPropagation();
            router.push(`/ciclos/${ciclo.id}`);
        }}>
              VER DETALLES
            </Button>
            <Button variant="outline" className="flex-1 font-bold uppercase" onClick={(e) => {
            e.stopPropagation();
...
//# sourceMappingURL=CicloCard.js.map