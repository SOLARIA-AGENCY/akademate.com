...
export function EntidadSelector({ onSelect, excluidas, entidadesUsadas }) {
    ...
    return (<div className="space-y-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
        {entidadesDisponibles.map((entidad) => (<button key={entidad.key} type="button" onClick={() => onSelect(entidad.key)} className={cn('flex flex-col items-center gap-2 p-3 border-2 rounded-lg', 'hover:border-primary hover:bg-accent', 'transition-colors cursor-pointer', 'focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2')}>
...
//# sourceMappingURL=EntidadSelector.js.map