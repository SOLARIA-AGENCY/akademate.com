...
export function SedeListItem({ sede, onClick, className }) {
    ...
    return (<div className={`flex items-center h-20 pr-4 bg-card border-y border-r rounded-lg overflow-hidden hover:shadow-md transition-shadow duration-150 cursor-pointer ${className || ''}`} onClick={onClick}>
      ...
      <div className="flex-shrink-0 h-full">
        <img src={sede.imagen} alt={sede.nombre} className="h-full w-20 object-cover"/>
      </div>
      ...
      <div className="flex items-center flex-1 gap-3 pl-4">
        ...
        <Button size="sm" className="bg-[#ff2014] hover:bg-[#ff2014]/90 text-white text-xs font-semibold uppercase tracking-wide shrink-0 h-7 px-3" onClick={(e) => {
            e.stopPropagation();
            onClick?.();
        }}>
          VER
...
//# sourceMappingURL=SedeListItem.js.map