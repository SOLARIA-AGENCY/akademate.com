...
export function ConvocationCard({ instance, onClick, className }) {
    ...
    return (<Card className={`convocation-card cursor-pointer hover:shadow-lg transition-all duration-300 overflow-hidden border-2 ${typeConfig.borderColor} ${className || ''}`} onClick={onClick}>
      <div className={`h-2 ${typeConfig.bgColor}`}/>

      <CardContent className="p-6 space-y-3 flex flex-col">
        {/* Header with Type Badge */}
        <div className="space-y-2">
          <Badge className={`${typeConfig.bgColor} ${typeConfig.hoverColor} text-white text-xs font-bold uppercase tracking-wide`}>
            {typeConfig.label}
          </Badge>

          {/* Título - ALTURA FIJA, 1 LÍNEA con ellipsis */}
          <div className="h-7 overflow-hidden">
            <h3 className="font-bold text-lg leading-7 uppercase truncate" title={instance.nombreCurso}>
        ...
        <div className="grid grid-cols-2 gap-3 py-3 border-y">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            {instance.precioConDescuento ? (<div className="flex items-center gap-1">
              ...
              </div>) : instance.precio === 0 ? (<span className="font-bold text-green-600 text-xs uppercase">
                100% SUBVENCIONADO
              </span>) : (<span className={`font-bold ${typeConfig.textColor}`}>
                {instance.precio}€
              </span>)}
          </div>
        ...
        {instance.subvencionado !== 'no' && instance.entidadesFinanciadoras.length > 0 && (<div className="pt-2 border-t">
            <p className="text-xs text-muted-foreground mb-2">Financiado por:</p>
            <div className="flex gap-2 flex-wrap items-center">
              {instance.entidadesFinanciadoras.map((entidad) => (<div key={entidad.id} className="px-2 py-1 bg-secondary rounded text-xs font-medium" title={entidad.nombre}>
                  {entidad.nombre}
                </div>))}
            </div>
          </div>)}
        ...
        <Button className={`w-full ${typeConfig.bgColor} ${typeConfig.hoverColor} text-white font-bold uppercase tracking-wide shadow-md transition-all duration-300 mt-auto`} onClick={(e) => {
            e.stopPropagation();
            onClick?.();
        }}>
          VER CURSO
...
//# sourceMappingURL=ConvocationCard.js.map