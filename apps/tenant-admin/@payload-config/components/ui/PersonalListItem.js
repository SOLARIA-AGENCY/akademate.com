...
export function PersonalListItem({ teacher, onClick, className }) {
    ...
    return (<div className={`flex items-center h-20 pr-4 bg-card border-y border-r rounded-lg overflow-hidden hover:shadow-md transition-shadow duration-150 cursor-pointer ${className || ''}`} onClick={onClick}>
      ...
      <div className="flex-shrink-0 w-20 h-20 overflow-hidden rounded-full bg-muted ml-4">
        <img src={teacher.photo} alt={`${teacher.first_name} ${teacher.last_name}`} className="w-full h-full object-cover"/>
      </div>
      ...
      <div className="flex items-center flex-1 gap-3 pl-4">
        ...
        <div className="hidden lg:flex flex-col gap-0.5 text-xs min-w-[140px]">
          {teacher.specialties.slice(0, 2).map((specialty, idx) => (<span key={idx} className="text-muted-foreground truncate leading-tight">
              • {specialty}
            </span>))}
        </div>
        ...
        <div className="hidden lg:block w-[100px] flex justify-center">
          <Badge className={`${teacher.active
            ...
            : 'bg-gray-500 hover:bg-gray-600'} text-white text-[10px] font-semibold uppercase tracking-wide whitespace-nowrap px-2.5 py-1 leading-tight`}>
        ...
        <Button size="sm" className="bg-primary hover:bg-primary/90 text-white text-xs font-semibold uppercase tracking-wide shrink-0 h-7 px-3" onClick={(e) => {
            e.stopPropagation();
            onClick?.();
        }}>
          VER
...
//# sourceMappingURL=PersonalListItem.js.map