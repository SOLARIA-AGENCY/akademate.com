 ...
 */
export function MockDataIndicator({ label = 'Datos mock', size = 'sm', variant = 'badge', className = '' }) {
    const sizeClasses = {
    ...
    if (variant === 'overlay') {
        return (<div className={`absolute inset-0 bg-gray-900/60 backdrop-blur-[1px] flex items-center justify-center z-10 rounded-lg ${className}`}>
    ...
    if (variant === 'banner') {
        return (<div className={`w-full bg-gray-100 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-lg px-4 py-2 flex items-center gap-2 ${className}`}>
 ...
 */
export function withMockIndicator(WrappedComponent, indicatorProps) {
 ...
 */
export function MockDataCard({ children, title, label = 'Datos mock', className = '' }) {
    ...
    `}>
      ...
      <div className="opacity-60 grayscale-[30%] p-4">
        {title && (<h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-3">
            {title}
          </h3>)}
        {children}
      </div>
...
//# sourceMappingURL=MockDataIndicator.js.map