      ...
      {value.length > 0 && (<div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2">
            {value.map((entry, index) => {
                ...
                return (<Badge key={index} variant="secondary" className="flex items-center gap-2 py-1.5 px-3">
                  <span className="font-semibold">{dayLabel}</span>
                  <span className="text-xs">
                    {entry.startTime} - {entry.endTime}
                  </span>
                  <button type="button" onClick={() => handleRemoveEntry(index)} className="ml-1 hover:bg-destructive/20 rounded-full p-0.5">
                    <X className="h-3 w-3"/>
                  </button>
                </Badge>);
            })}
      ...
      <div className="space-y-3 p-4 border rounded-lg bg-muted/30">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="space-y-2">
            ...
            <Select value={newEntry.startTime} onValueChange={(time) => setNewEntry({ ...newEntry, startTime: time })}>
          ...
          <div className="space-y-2">
            ...
            <Select value={newEntry.endTime} onValueChange={(time) => setNewEntry({ ...newEntry, endTime: time })}>
        ...
        <Button type="button" variant="outline" size="sm" onClick={handleAddEntry} disabled={!isValidEntry} className="w-full">
