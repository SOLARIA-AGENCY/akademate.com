...
export function CursoCicloCard({ curso, cicloImagen, cicloColor, className }) {
    return (<Card className={`hover:shadow-md transition-all duration-300 overflow-hidden border ${className}`} style={{ maxHeight: '280px' }}>
      <CardContent className="p-0">
        {/* Image (heredada del ciclo, más pequeña) */}
        <div className="w-full h-24 overflow-hidden bg-gray-100">
          <img src={cicloImagen} alt={curso.nombre} className="w-full h-full object-cover opacity-70"/>
        </div>
...
//# sourceMappingURL=CursoCicloCard.js.map