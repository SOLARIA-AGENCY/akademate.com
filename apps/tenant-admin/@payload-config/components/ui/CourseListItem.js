...
export function CourseListItem({ course, onClick, className }) {
    const typeConfig = COURSE_TYPE_CONFIG[course.tipo] || COURSE_TYPE_CONFIG.privados;
    return (<div className={`flex items-center h-24 px-6 bg-card rounded-lg overflow-hidden hover:shadow-md transition-shadow duration-150 cursor-pointer ${className || ''}`} onClick={onClick}>
      {/* Thumbnail */}
      <div className="flex-shrink-0 h-16 w-24 rounded overflow-hidden" style={{ backgroundColor: '#f5f5f5' }}>
        <img src={course.imagenPortada} alt={course.nombre} className="h-full w-full object-cover"/>
      </div>
      ...
      <div className="flex items-center flex-1 gap-8 pl-6">
        ...
        <div className="hidden lg:flex items-center justify-center min-w-[140px]">
          <Badge className={`${typeConfig.bgColor} ${typeConfig.hoverColor} text-white text-xs font-bold uppercase tracking-wide px-3 py-1`}>
            {typeConfig.label}
          </Badge>
        </div>
        ...
        <div className="hidden md:flex flex-col gap-1 min-w-[140px] items-end">
          <div className="flex items-center gap-2">
            ...
            {course.precioReferencia !== undefined && course.precioReferencia > 0 ? (<span className={`font-bold text-lg ${typeConfig.textColor}`}>
                {course.precioReferencia}€
              </span>) : (<span className="font-bold text-lg text-green-600">
        ...
        {/* Action Button */}
        <Button size="sm" className={`${typeConfig.bgColor} ${typeConfig.hoverColor} text-white text-xs font-bold uppercase tracking-wide shrink-0 h-8 px-4`} onClick={(e) => {
            e.stopPropagation();
            onClick?.();
        }}>
          VER CURSO
