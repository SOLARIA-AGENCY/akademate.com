...
export function SubvencionItem({ subvencion, onUpdate, onRemove }) {
    const entidadInfo = getEntidadInfo(subvencion.entidad);
    return (<div className="flex items-start gap-4 p-4 bg-card border rounded-lg">
      {/* Logo de la entidad */}
      ...
      <div className="flex-1 min-w-0 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <Input type="number" min="0" max="100" value={subvencion.porcentaje} onChange={(e) => {
          ...
          <div className="flex items-end">
            <div className="flex items-center space-x-2">
              <Switch id={`activa-${subvencion.id}`} checked={subvencion.activa} onCheckedChange={(activa) => onUpdate({ ...subvencion, activa })}/>
        ...
        <div>
          ...
          <Textarea value={subvencion.requisitos || ''} onChange={(e) => onUpdate({ ...subvencion, requisitos: e.target.value })} placeholder="Ej: Trabajadores en activo, autónomos..." rows={2} className="text-xs mt-1"/>
        </div>

        {/* URL info (opcional) */}
        <div>
          <Label className="text-xs">URL de información (opcional)</Label>
          <Input type="url" value={subvencion.urlInfo || ''} onChange={(e) => onUpdate({ ...subvencion, urlInfo: e.target.value })} placeholder="https://..." className="h-8 text-xs mt-1"/>
        </div>
      </div>

      {/* Botón eliminar */}
      <Button variant="ghost" size="icon" onClick={onRemove} className="text-destructive hover:bg-destructive/10 flex-shrink-0">
        <Trash2 className="w-4 h-4"/>
      </Button>
    </div>);
}
//# sourceMappingURL=SubvencionItem.js.map