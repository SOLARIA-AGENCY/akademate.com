...
export function PublicInfoRows({
  ...
  return (
    <div className={cn('grid gap-3 rounded-xl bg-slate-50 p-4', className)}>
      {items.map((item) => (
        <div key={item.label} className="grid gap-1 text-sm sm:grid-cols-[120px_minmax(0,1fr)] sm:items-start sm:gap-4">
          <span className="font-bold text-slate-950">{item.label}</span>
          <span className="min-w-0 leading-relaxed text-slate-600 sm:text-right">{item.value}</span>
        </div>
      ))}
    </div>
  )
