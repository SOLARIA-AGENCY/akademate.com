...
import { LayoutDashboard, BookOpen, Calendar, CalendarDays, Users, UserCircle, Building2, FileText, UserPlus, ListTodo, Megaphone, Sparkles, FileEdit, Newspaper, HelpCircle, MessageSquareQuote, Image, BarChart3, Settings, ChevronDown, ChevronLeft, ChevronRight, GraduationCap, MapPin, Shield, List, Lock, Briefcase, Monitor, Globe, FileInput, Eye, UserCog, CreditCard, } from 'lucide-react';
...
function SubMenuItem({ subItem, pathname }) {
    ...
    if (hasNestedItems) {
        return (<>
        <button onClick={() => setNestedOpen(!nestedOpen)} className="w-full flex items-center gap-2 rounded-md px-3 py-2 text-sm transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground">
          <SubIcon className="h-4 w-4 shrink-0" style={{ color: '#F2014B' }}/>
          <span className="flex-1 text-left">{subItem.title}</span>
          <ChevronDown className={`h-3 w-3 transition-transform ${nestedOpen ? 'rotate-180' : ''}`}/>
        </button>
        {nestedOpen && (<ul className="ml-4 mt-1 space-y-1 border-l border-sidebar-border pl-4">
            {subItem.items?.map((nestedItem) => {
                    const NestedIcon = nestedItem.icon;
                    const isNestedActive = pathname === nestedItem.url;
                    return (<li key={nestedItem.title}>
                  <Link href={nestedItem.url} className={`flex items-center gap-2 rounded-md px-3 py-2 text-sm transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground ${isNestedActive
    ...
    return (<Link href={subItem.url} className={`flex items-center gap-2 rounded-md px-3 py-2 text-sm transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground ${isSubActive
            ? 'bg-sidebar-accent text-sidebar-accent-foreground'
            : ''}`}>
      <SubIcon className="h-4 w-4 shrink-0" style={{ color: '#F2014B' }}/>
...
export function AppSidebar({ isCollapsed = false, onToggle }) {
    ...
    const toggleSection = (title) => {
        if (isCollapsed)
            return;
        setOpenSections((prev) => prev.includes(title)
            ? prev.filter((item) => item !== title)
            : [title] // Only keep the new section open, close all others
        );
    };
    return (<div className="flex h-full flex-col overflow-hidden bg-card text-sidebar-foreground">
      ...
      <div className="flex h-16 items-center border-b border-sidebar-border px-4 overflow-hidden">
        <div className={`flex items-center w-full transition-all duration-300 ease-in-out ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
          ...
          <div className="w-9 h-9 rounded-md bg-white flex items-center justify-center flex-shrink-0 p-0.5">
            <NextImage src={logoUrl} alt={academyName} width={32} height={32} className="w-8 h-8 object-contain"/>
          </div>
          <span className={`text-lg font-semibold text-sidebar-foreground whitespace-nowrap transition-all duration-300 ease-in-out ${isCollapsed ? 'w-0 opacity-0 overflow-hidden' : 'w-auto opacity-100'}`}>
            CEP Formación
          </span>
        </div>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto py-2 px-2">
        <ul className="space-y-1">
          {menuItems.map((item) => {
            ...
            const SectionSeparator = item.sectionBefore ? (<li className="pt-4 pb-2 overflow-hidden">
                <div className="relative flex items-center justify-center">
                  {/* Text label - fades out when collapsed */}
                  <span className={`px-3 text-xs font-bold uppercase tracking-wider whitespace-nowrap transition-all duration-300 ease-in-out ${isCollapsed ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100 w-auto'}`} style={{ color: '#F2014B' }}>
                    {item.sectionBefore}
                  </span>
                  {/* Line separator - fades in when collapsed */}
                  <div className={`w-8 border-t transition-all duration-300 ease-in-out ${isCollapsed ? 'opacity-100' : 'opacity-0 w-0'}`} style={{ borderColor: '#F2014B' }}/>
                </div>
              </li>) : null;
            if (!hasSubItems) {
                return (<React.Fragment key={item.title}>
                  {SectionSeparator}
                  <li>
                    <Link href={item.url} className={`flex items-center rounded-md py-2 text-sm transition-all duration-300 ease-in-out hover:bg-sidebar-accent hover:text-sidebar-accent-foreground ${isActive ? 'bg-sidebar-accent text-sidebar-accent-foreground' : ''} ${isCollapsed ? 'justify-center px-2' : 'gap-3 px-3'}`} title={isCollapsed ? item.title : undefined}>
            ...
            return (<React.Fragment key={item.title}>
                {SectionSeparator}
                <li>
                  <button onClick={() => toggleSection(item.title)} className={`w-full flex items-center rounded-md py-2 text-sm transition-all duration-300 ease-in-out hover:bg-sidebar-accent hover:text-sidebar-accent-foreground ${isCollapsed ? 'justify-center px-2' : 'gap-3 px-3'}`} title={isCollapsed ? item.title : undefined}>
                    <Icon className="h-5 w-5 shrink-0" style={{ color: '#F2014B' }}/>
                    <span className={`flex-1 text-left whitespace-nowrap transition-all duration-300 ease-in-out ${isCollapsed ? 'w-0 opacity-0 overflow-hidden' : 'w-auto opacity-100'}`}>
                      {item.title}
                    </span>
                    <ChevronDown className={`h-4 w-4 shrink-0 transition-all duration-300 ease-in-out ${isOpen ? 'rotate-180' : ''} ${isCollapsed ? 'w-0 opacity-0 overflow-hidden' : 'opacity-100'}`}/>
                  </button>
                  {/* Submenu with smooth height transition */}
                  <div className={`overflow-hidden transition-all duration-300 ease-in-out ${!isCollapsed && isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
      ...
      <div className="border-t border-sidebar-border mt-auto">
        ...
        {onToggle && (<div className={`py-2 flex items-center border-b border-sidebar-border ${isCollapsed ? 'justify-center px-2' : 'justify-end px-4'}`}>
            <button onClick={onToggle} className="flex h-8 w-8 items-center justify-center rounded-md hover:bg-sidebar-accent transition-colors" title={isCollapsed ? 'Expandir sidebar' : 'Colapsar sidebar'} style={{ color: '#F2014B' }}>
              {isCollapsed ? (<ChevronRight className="h-4 w-4"/>) : (<ChevronLeft className="h-4 w-4"/>)}
            </button>
          </div>)}

        {/* Help Section - Always visible with smooth transitions */}
        <Link href="/ayuda" className={`flex items-center hover:bg-sidebar-accent transition-all duration-300 ease-in-out h-[46px] ${isCollapsed ? 'justify-center px-2' : 'gap-3 px-4'}`} title={isCollapsed ? 'Ayuda y Documentación' : undefined}>
          <HelpCircle className="h-3.5 w-3.5 shrink-0" style={{ color: '#F2014B' }}/>
          <div className={`min-w-0 transition-all duration-300 ease-in-out ${isCollapsed ? 'w-0 opacity-0 overflow-hidden' : 'flex-1 opacity-100'}`}>
