...
const STAFF_FULL_NAME = 'Lucía Corominas Pérez';
const STAFF_PHOTO_FILENAME = 'lucia-corominas.webp';
const STAFF_PHOTO_ALT = 'Lucía Corominas, docente de Auxiliar de Farmacia, Nutricosmética, Clínicas Estéticas y Entrenamiento Personal';
const STAFF_ALIASES = ['Lucía', 'Lucía Corominas', 'Lucia Corominas', 'Lucía Corominas Pérez'];
const DETECTED_COURSES = [
  'Auxiliar de Farmacia y Parafarmacia + Dermocosmética',
  'Nutricosmética',
  'Auxiliar en Clínicas Estéticas',
...
const COURSE_ALIAS_GROUPS = [
  [
    ...
    'Auxiliar de Farmacia y Parafarmacia + Dermocosmética',
    'Farmacia y Dermocosmética',
    'Aux Farmacia, Parafarmacia y Dermocosmética',
  ],
  [
    'Nutricosmética',
    'Nurricosmetica',
  ...
  [
    ...
    'Auxiliar en Clínicas Estéticas',
    'Aux. en Clínicas Estéticas',
  ],
  [
    'Entrenamiento Personal',
  ],
].map((group) => new Set(group.map(normalizeText)));
...
async function findCampuses(payload: PayloadClient, tenantId: number) {
  const campuses = await fetchAll(payload, 'campuses', { tenant: { equals: tenantId } });
  return campuses
    .filter((campus) => ['sede-norte', 'sede-santa-cruz'].includes(String(campus.slug ?? '')))
...
async function findLuciaStaff(payload: PayloadClient) {
...
async function ensureStaff(payload: PayloadClient, tenantId: number, photoId: unknown, apply: boolean) {
  const existing = await findLuciaStaff(payload);
  const assignedCampuses = await findCampuses(payload, tenantId);
  const data = {
    staff_type: 'profesor',
    first_name: 'Lucía',
    last_name: 'Corominas Pérez',
    ...
    notes: 'Especialidades CEP: Auxiliar de farmacia, Parafarmacia, Dermocosmética, Clínicas estéticas, Entrenamiento personal, Nutricosmética',
  };
...
function matchesCourseAlias(courseName: unknown) {
  const normalized = normalizeText(String(courseName ?? ''));
  return COURSE_ALIAS_GROUPS.some((aliases) => aliases.has(normalized));
}
...
async function findCourses(payload: PayloadClient, tenantId: number) {
  ...
  return courses.filter((course) => matchesCourseAlias(course.name));
}
...
async function main() {
  ...
  console.log(`Lucía Corominas sync (${result.mode})`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  void main().catch((error) => {
    console.error('sync-lucia-corominas failed:', error);
    process.exit(1);
  });
}
