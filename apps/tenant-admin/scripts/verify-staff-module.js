...
async function verifyStaffModule() {
    ...
    `;
        const professors = staffRecords.filter((s) => s.staff_type === 'profesor');
        const admins = staffRecords.filter((s) => s.staff_type === 'administrativo');
        ...
        const withoutPhotos = staffRecords.filter((s) => !s.photo_id);
        if (withoutPhotos.length > 0) {
            console.log(`   ⚠ ${withoutPhotos.length} staff without photos:`);
            withoutPhotos.forEach((s) => console.log(`      - ${s.full_name}`));
        }
        else {
        ...
        console.log(`   Professors: ${professors.length}`);
        console.log(`   Administrative: ${admins.length}`);
        console.log(`   Photos Linked: ${staffRecords.filter((s) => s.photo_id).length}/${staffRecords.length}`);
...
verifyStaffModule();
//# sourceMappingURL=verify-staff-module.js.map