...
function SidebarProvider({
  ...
  React.useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const isInputField =
        ...
        (activeElement?.getAttribute('role') || '').includes('button') ||
  ...
  React.useEffect(() => {
    if (openProp === undefined && !isMobile) {
      document.cookie = `${SIDEBAR_COOKIE_NAME}=${open}; max-age=${SIDEBAR_COOKIE_MAX_AGE}`
    }
...
function SidebarMenuSkeleton({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div data-slot="sidebar-menu-skeleton" data-sidebar="menu-skeleton" className={cn('flex items-center gap-2 px-2 py-1', className)} {...props}>
      <div data-slot="sidebar-menu-skeleton-icon" className="bg-sidebar-foreground/10 rounded-sm" />
      <div data-slot="sidebar-menu-skeleton-text" className="bg-sidebar-foreground/10 rounded-sm" />
    </div>
  )
}

function SidebarMenuSub({ className, ...props }: React.ComponentProps<'ul'>) {
  return <ul data-slot="sidebar-menu-sub" data-sidebar="menu-sub" className={cn('border-sidebar-border mx-3.5 flex min-w-0 translate-x-px flex-col gap-1 border-l px-2.5 py-0.5', className)} {...props} />
}
...
function SidebarMenuSubButton({ className, isActive = false, ...props }: React.ComponentProps<'button'> & { isActive?: boolean }) {
  return (
    <button
