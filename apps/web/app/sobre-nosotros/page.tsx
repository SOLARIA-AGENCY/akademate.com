...
export default function AboutPage() { return <div className="min-h-screen bg-white text-[#071633]"><Header /><main id="content"><section className="grid min-h-[calc(100dvh-73px)] items-stretch lg:grid-cols-2"><div className="flex items-center px-4 py-20 sm:px-8 lg:px-[max(2rem,calc((100vw-80rem)/2))]"><div className="max-w-xl"><p className="section-kicker">Meet Akademate</p><h1 className="mt-5 text-5xl font-semibold tracking-[-0.055em] sm:text-7xl">We&apos;re building the platform ambitious academies deserve.</h1><p className="mt-7 text-lg leading-8 text-slate-600">A product of {legalCompany.name}, Akademate brings the commercial engine, learning experience and daily operation of an academy into one connected product.</p><Link href="/contacto?asunto=demo" className="button-primary-dark mt-9">Build the future with us <ArrowRight className="h-4 w-4" aria-hidden="true" /></Link></div></div><div className="relative min-h-[480px]"><Image src="/images/marketing/about-founder-campus.jpg" alt="Academy founder walking through a connected learning campus" fill priority sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" /></div></section><section className="bg-[#071633] px-4 py-20 text-white sm:px-6 lg:px-8 lg:py-28"><div className="mx-auto max-w-7xl"><p className="text-sm font-semibold uppercase tracking-[0.22em] text-blue-200">Why we exist</p><h2 className="mt-5 max-w-4xl text-4xl font-semibold tracking-[-0.045em] sm:text-6xl">Better operations create more space for great teaching and real human connection.</h2><div className="mt-14 grid border-y border-white/15 md:grid-cols-3">{principles.map(({ icon: Icon, title, text }) => <article key={title} className="border-b border-white/15 py-8 md:border-b-0 md:border-r md:px-8 first:md:pl-0 last:md:border-r-0"><Icon className="h-7 w-7 text-blue-300" aria-hidden="true" /><h3 className="mt-8 text-xl font-semibold">{title}</h3><p className="mt-3 text-sm leading-6 text-blue-100/65">{text}</p></article>)}</div></div></section></main><Footer /></div> }
